...
import SubCategory from "../models/master/subCategory.model";
...
import { VendorOrder } from "../models/VendorRelated";
...
const normalizeProductData = (data: any): any => {
  const normalized = {
    ...data,
  };
  ...
  if (Array.isArray(normalized.metaKeywords)) {
    normalized.metaKeywords = normalized.metaKeywords
      .map((keyword: any) => normalizeString(keyword))
      .filter((k: any) => k && k.toLowerCase() !== "string");
  }

  if (Array.isArray(normalized.tags)) {
    normalized.tags = normalized.tags
      .map((tag: any) => normalizeString(tag))
      .filter((t: any) => t && t.toLowerCase() !== "string");
  }
...
export const findProductByIdOrIdentifier = async (idOrIdentifier: string) => {
  ...
  if (mongoose.Types.ObjectId.isValid(cleanId)) {
    const prod = await Product.findById(cleanId);
    if (prod) return prod;
  }
  ...
  if (!isNaN(num) && num > 0 && String(num) === cleanId) {
    const prods = await Product.find().sort({ createdAt: -1 }).skip(num - 1).limit(1);
...
const validateCategory = async (categoryIdOrName: any) => {
  if (!categoryIdOrName) {
...
const validateSubCategory = async (subCategoryIdOrName: any, categoryId: any) => {
...
const validateBrand = async (brandIdOrName: any) => {
  if (!brandIdOrName) {
    return null;
  }

  let brand: any = null;
  if (mongoose.Types.ObjectId.isValid(brandIdOrName)) {
    brand = await Brand.findById(brandIdOrName);
  }
...
const validateUnit = async (unitIdOrName: any) => {
  if (!unitIdOrName) {
  ...
  let unit: any = null;
  if (mongoose.Types.ObjectId.isValid(unitIdOrName)) {
    unit = await Unit.findById(unitIdOrName);
  }
...
const validateTax = async (taxIdOrName: any) => {
  if (!taxIdOrName) {
    return null;
  }

  let tax: any = null;
  if (mongoose.Types.ObjectId.isValid(taxIdOrName)) {
    tax = await Tax.findById(taxIdOrName);
  }
  ...
  if (!tax) {
    ...
    if (rateMatch) {
      ...
      tax = await Tax.findOne({ taxRate: rate, status: "active" });
    }
  }
...
const validateAttributes = async (attributes: any) => {
  if (!Array.isArray(attributes) || attributes.length === 0) {
    return [];
  }

  const validObjectIds = attributes.filter((a) => mongoose.Types.ObjectId.isValid(a));
...
const validateVariantAttributeValues = async (
  variants: any,
  ...
  const validObjectIds = (selectedAttributeIds || []).filter((a: any) => mongoose.Types.ObjectId.isValid(a));
  if (validObjectIds.length === 0) {
    return;
  }
  ...
  const selectedIds = new Set(
    validObjectIds.map((id: any) => id.toString())
  );
  ...
  for (const variant of variants) {
    ...
    for (const variantAttribute of variant.attributes) {
      if (!variantAttribute.attribute) continue;
...
const checkSlugDuplicate = async (slug: any, excludeId: any = null) => {
  if (!slug) {
    return;
  }
...
const checkBarcodeDuplicate = async (barcode: any, excludeId: any = null) => {
  if (!barcode) {
    return;
  }
...
const checkVariantSkuDuplicates = (variants: any) => {
...
export const seedCatalogIfEmpty = async (): Promise<void> => {
  ...
  // Ensure default unit
  let unit = await Unit.findOne({ status: "active" });
  if (!unit) {
    unit = await Unit.create({
      unitCode: "UNT-001",
      unitName: "Piece",
  ...
  // Ensure default tax
  let tax = await Tax.findOne({ status: "active" });
  if (!tax) {
    tax = await Tax.create({
      taxCode: "TAX-001",
      taxName: "GST 18%",
  ...
  let warehouse = await Warehouse.findOne({ status: "active" });
  if (!warehouse) {
  ...
  for (let i = 0; i < categoryNames.length; i++) {
    ...
    let cat = await Category.findOne({ categoryName: cName });
    if (!cat) {
    ...
    catMap.set(cName, cat._id as mongoose.Types.ObjectId);
  }
  ...
  for (let i = 0; i < brandNames.length; i++) {
    ...
    let b = await Brand.findOne({ brandName: bName });
    if (!b) {
    ...
    brandMap.set(bName, b._id as mongoose.Types.ObjectId);
  }
  ...
  for (const item of catalogSeeds) {
    const existing = await Product.findOne({ productCode: item.code });
    if (existing) {
      continue;
    }
...
export const createProduct = async (data: any, user?: any) => {
  ...
  const validationErrors = validateCreateProduct(normalizedData);
...
export const getProducts = async (query: GetProductsQuery = {}) => {
  ...
  if (category && String(category).toLowerCase() !== "all") {
    if (mongoose.Types.ObjectId.isValid(category)) {
      ...
      const catDoc = await Category.findOne({ categoryName: new RegExp(`^${escapeRegex(String(category).trim())}$`, "i") });
  ...
  if (brand && String(brand).toLowerCase() !== "all") {
    if (mongoose.Types.ObjectId.isValid(brand)) {
      ...
      const brandDoc = await Brand.findOne({ brandName: new RegExp(`^${escapeRegex(String(brand).trim())}$`, "i") });
  ...
  for (const s of stockRecords) {
    ...
    if (idKey) stockMap.set(idKey, (stockMap.get(idKey) || 0) + qty);
    if (skuKey) stockMap.set(skuKey, (stockMap.get(skuKey) || 0) + qty);
  }

  const products = rawProducts.map((prod) => {
    const obj: any = prod.toObject ? prod.toObject() : { ...prod };
...
export const getProductById = async (id: string) => {
  const product = await findProductByIdOrIdentifier(id);
  ...
  const obj: any = populatedProduct ? populatedProduct.toObject() : product.toObject();
...
export const updateProduct = async (id: string, data: any, user?: any) => {
  const existingProduct = await findProductByIdOrIdentifier(id);
  ...
  const normalizedData = normalizeProductData(data);
  const validationErrors = validateUpdateProduct(normalizedData);
  ...
  return await getProductById(existingProduct._id.toString());
};
...
export const updateProductStatus = async (id: string, status: string, user?: any) => {
  const normalizedStatus = String(status).toLowerCase();
  if (!PRODUCT_STATUSES.includes(normalizedStatus)) {
    throw createError(`Status must be one of: ${PRODUCT_STATUSES.join(", ")}.`, 400);
  }
...
export const duplicateProduct = async (id: string, user?: any) => {
  const existingProduct = await findProductByIdOrIdentifier(id);
  if (!existingProduct) {
    throw createError("Product not found.", 404);
  }
  ...
  const duplicateData: any = existingProduct.toObject();
  ...
  if (Array.isArray(duplicateData.variants)) {
    duplicateData.variants = duplicateData.variants.map((v: any, i: number) => ({
      ...v,
  ...
  const clonedProduct = await Product.create(duplicateData);
  return await getProductById(clonedProduct._id.toString());
};
...
export const bulkUpdateStatus = async (ids: string[], status: string, user?: any) => {
  const validationErrors = validateBulkStatus({ ids, status });
  if (Object.keys(validationErrors).length > 0) {
  ...
  const result = await Product.updateMany(
    { _id: { $in: ids } },
    { $set: { status: normalizedStatus, updatedBy: user?._id || null } }
  );
...
export const bulkUpdatePublish = async (ids: string[], isPublished: boolean, user?: any) => {
  const validationErrors = validateBulkPublish({ ids, isPublished });
  if (Object.keys(validationErrors).length > 0) {
  ...
  const result = await Product.updateMany(
    { _id: { $in: ids } },
    { $set: { isPublished: Boolean(isPublished), updatedBy: user?._id || null } }
  );
...
export const bulkDelete = async (ids: string[]) => {
  const validationErrors = validateBulkDelete({ ids });
  if (Object.keys(validationErrors).length > 0) {
    throw createError("Bulk delete validation failed.", 400, validationErrors);
  }

  const result = await Product.deleteMany({ _id: { $in: ids } });
  await InventoryStock.deleteMany({ productId: { $in: ids } });
...
export const getProductOrders = async (productId: string) => {
  const product = await findProductByIdOrIdentifier(productId);
  if (!product) {
    throw createError("Product not found.", 404);
  }
  ...
  const orders = await VendorOrder.find(query).sort({ createdAt: -1 }).limit(10).lean();
...
export const getProductStats = async () => {
  ...
  const allStock = await InventoryStock.find({}).lean();
  let lowStockCount = 0;
  let outOfStockCount = 0;
  for (const s of allStock) {
    if (s.availableStock <= 0) outOfStockCount++;
    else if (s.availableStock < 10) lowStockCount++;
  }
...
export const deleteProduct = async (id: string) => {
  ...
  await InventoryStock.deleteMany({ productId: product._id });
